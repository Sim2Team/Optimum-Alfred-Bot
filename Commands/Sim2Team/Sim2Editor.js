...
module.exports = {
	...
	Handler(Message) {
		const CMD = Message.Value.toLowerCase(); // Save the argument to a variable for more efficiency.

		if (CMD.length < 1) Message.channel.send("No \"sub\" command for Sim2Editor provided! Type \"-h\" or \"-help\" to the command for a list of Sim2Editor commands.");
		else {
